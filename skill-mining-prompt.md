...
- **Mid-flow corrections** — every place I corrected the AI: "no, we always…", "that should have been…", "you should have asked…", "why didn't you use…". Grep the user turns for these patterns. Each one is a rule the AI gets wrong by default, which makes it the highest-leverage thing to encode.
...
- Knowledge that already lives in my notes/memory as prose, but the AI still re-derives the *executable procedure* every session. (Operationalizing notes you already maintain is the cheapest high-value skill.)

**⚠ The trap that ruins this exercise:** confusing what I *happen to be working on* with how the environment *works*. First passes reliably surface project-storyline workflows that look impressive but help nobody else. Litmus-test every candidate: **"Would this matter to a different person doing a different task in this environment?"** If it's specific to one project's storyline, it's context — drop it. (Solo user? The test becomes: "will this matter in my next unrelated project?")
...
Then run an **adversarial critique pass** with fresh eyes (a subagent works well): What did the synthesis miss? What's over-proposed — a one-off dressed up as a pattern? What's mis-tiered? Verify the critique's claims before accepting them, apply the corrections, and show me the final catalog to pick from.
...
3. **Fix what the eval exposes** — especially any fact the eval proves stale. An eval that finds a bug in your skill has paid for itself.
4. **Ship in stages** — install locally, dogfood for a week, then publish to the team's plugin/marketplace. Schedule the publish reminder *now* so it actually happens.

Throughout: terse updates between phases; evidence over inference; and when my correction contradicts your finding, my correction wins — encode it.

---

# Collaborative mode — mining a whole team's history

Solo mining has a blind spot: your sessions only contain *your* corrections and *your* corner of the system. The teammate who set up CI has conventions in their history that appear in nobody else's. But **raw transcripts must never leave each person's machine** — they contain echoed secrets, tokens, customer data, and private context. So collaborate map-reduce style: each person mines locally, only a sanitized findings file travels, and one maintainer merges.
...
Contributors submit the reviewed file via PR to a shared location (a `findings/` dir in your plugins repo works — PR review doubles as a second privacy check).
...
>
> Then continue with Phases 3–5 of the solo prompt above: verify every merged fact against live sources yourself (contributor evidence is testimony, not proof), build the tiered catalog, run the adversarial critique, and build the winner.
...
- **Coverage** — the CI owner's, data engineer's, and on-call engineer's histories each contain a slice nobody else has. The union is the org's actual operating manual.
